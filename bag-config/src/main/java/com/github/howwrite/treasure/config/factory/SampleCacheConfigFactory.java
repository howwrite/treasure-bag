...
public class SampleCacheConfigFactory implements ConfigFactory {
    ...
    LoadingCache<Pair<String, String>, Optional<String>> kkvCache = CacheBuilder.newBuilder()
            .maximumSize(1000)
            .expireAfterAccess(10, TimeUnit.MINUTES)
            .build(new CacheLoader<>() {
                @Nonnull
                @Override
                public Optional<String> load(@Nonnull Pair<String, String> key) {
                    return Optional.ofNullable(ConfigFetcherProvider.provideConfigFetcher(key.getKey(), key.getValue()).readConfig(key.getKey(), key.getValue()));
                }
            });

    @Override
    public String readConfig(String namespace, String key) {
        return kkvCache.getUnchecked(Pair.of(namespace, key)).orElse(null);
    }
